              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                  <?php 
                    ...
                    foreach($Pagos as $_P){
                      ...
                      <tr>
                        ...
                        <td><?= $_P["montoTotal"] ?></td>
                        <?php if(isset($_P["idpagosF"])){ ?>
                        <td> <input type="checkbox" class="minimal" <?= ($_P["estatus"]=="1")? "checked" : "" ?> id="pago<?= $_P["idpagosF"] ?>" value="<?= $_P["estatus"] ?>" onchange="RelizarPago(<?= $_P["idpagosF"] ?>, this.value, <?= $_P["idcredito"] ?>)"> </td>
                        <?php } ?>
                        </tr>
                      <?php
                    }
                  ...
                  <tr>
                    <th></th>
                    <th>Total</th>
                    <th><?= ceil($_POST["cantinvert"]) ?></th>
                    <th><?= $TMontoInteres ?></th>
                    <th><?= $_POST["cantinvert"]+$TMontoInteres ?></th>
                  </tr>            
                </tbody>
              </table>
            </div>
